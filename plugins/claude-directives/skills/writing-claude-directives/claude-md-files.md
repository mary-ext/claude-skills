...
CLAUDE.md files bridge Claude's statelessness. They preserve context so humans don't re-explain
architectural intent every session.
...
Claude auto-reads CLAUDE.md from current directory up to root. Depth: typically one level (domain).
Occasionally two (subdomain). Rarely more.
...
Ensures user identity is verified exactly once at the system edge. All downstream services trust the
auth token without re-validating.
...
Every CLAUDE.md must include a "Last verified" date. Use Bash to get the actual date — don't
hallucinate:
...
- **Use `@./service.ts` syntax** to reference files. It force-loads them, burning tokens. Just name
  them — Claude reads on demand.
